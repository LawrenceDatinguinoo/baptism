...
function initInvitationCard() {
  ...
  function spawnSparkles() {
    ...
    for (let i = 0; i < sparkleCount; i += 1) {
      ...
      sparkle.className = 'invitation-sparkle';
      const dx = Math.round((Math.random() - 0.5) * 220);
      const dy = Math.round((Math.random() - 0.5) * 240);
      sparkle.style.setProperty('--dx', dx);
      sparkle.style.setProperty('--dy', dy);
      sparkle.style.left = `${50 + Math.random() * 12 - 6}%`;
      sparkle.style.top = `${50 + Math.random() * 12 - 6}%`;
  ...
  overlay.addEventListener('transitionend', (event) => {
    if (event.propertyName === 'opacity' && overlay.classList.contains('opened')) {
      overlay.style.display = 'none';
    }
  });
}

document.addEventListener('DOMContentLoaded', initInvitationCard);
